...
class MultiScaleDeformableAttnFunction(Function):
    ...
    def forward(
        ...
        # print(value.dtype, value_spatial_shapes.dtype, value_level_start_index.dtype, sampling_locations.dtype, attention_weights.dtype, im2col_step)
        value_level_start_index = value_level_start_index.to(torch.long)
...
class MultiScaleDeformableAttention(BaseModule):
    ...
    def forward(
        ...
        **kwargs,
        ...
        device = value.device
        spatial_shapes = spatial_shapes.to(dtype=torch.long, device=device)
        ...
        if reference_points.shape[-1] == 2:
            ...
            # sampling_locations = sampling_locations.to(torch.device)
            reference_points = reference_points.to(device)
            sampling_offsets = sampling_offsets.to(device)
