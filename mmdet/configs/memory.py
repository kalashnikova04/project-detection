...
class AvoidOOM:
    ...
    def retry_if_cuda_oom(self, func):
        ...
        def wrapped(*args, **kwargs):
            ...
            # Convert to FP16
            print(cast_tensor_type(args, dst_type=torch.half).size())
            fp16_args = cast_tensor_type(args, dst_type=torch.half).float()
            fp16_kwargs = cast_tensor_type(kwargs, dst_type=torch.half).float()
